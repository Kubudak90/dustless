...
/**
 * Card component for displaying and recovering a stuck asset
 * Memoized to prevent unnecessary re-renders when parent state changes
 */
export const StuckAssetCard = memo(function StuckAssetCard({ asset, targetChainId }: StuckAssetCardProps) {
  ...
  const handleGetQuotes = () => {
    ...
    if (isNativeETH) {
      ...
      // Use bridgeableAmount (balance - gas) to ensure enough for gas fees
      setFlow("bridge");
      getQuotes({
        fromChainId: asset.chainId,
        toChainId: targetChainId,
        amountWei: asset.bridgeableAmount || asset.balance,
      });
    } else {
      // For ERC-20, first get swap quotes
      // ERC-20 tokens don't need gas buffer (gas paid in native token)
      setFlow("swap");
      getSwapQuotes({
