 ...
 */
export const LiFiBridgeWidget = memo(function LiFiBridgeWidget({
  fromChainId,
  toChainId,
  fromToken,
  toToken,
  ...
  // Memoize config to prevent widget re-initialization
  const config = useMemo<WidgetConfig>(() => ({
    ...
    ...(fromToken && { fromToken }),
    ...(toToken && { toToken }),
  }), [fromChainId, toChainId, fromToken, toToken, variant]);
...
export const QuickBridge = memo(function QuickBridge({ fromChainId, toChainId, className }: QuickBridgeProps) {
  return (
    <LiFiBridgeWidget
